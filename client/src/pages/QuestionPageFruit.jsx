...
export default function QuestionPageFruit() {
    ...
    function onNextBtnClick() {
        ...
        let questionContainers = document.querySelectorAll(".question-radio-container-fruit");
        ...
        for (let questionIndex = 0; questionIndex < questionContainers.length; questionIndex++) {
            for (let radioIndex = 0; radioIndex < questionContainers[questionIndex].childNodes.length; radioIndex++) {
                if (questionContainers[questionIndex].childNodes[radioIndex].checked) {
                    increaseWeight(questionFruit[questionIndex]["qWeight"][radioIndex]["type"], questionFruit[questionIndex]["qWeight"][radioIndex]["weight"]);
                }
            }
        }
        ...
        <>
            <div className="question-section-wrapper">
                {questionFruit.map((element, index) => {
                    return <Question type="fruit" index={index} qString={questionFruit[index]["qString"]} qWeightArray={questionFruit[index]["qWeight"]}></Question>;
                })}
            </div>
