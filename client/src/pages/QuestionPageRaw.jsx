...
import questionRaw from "../assets/questionRaw";
...
export default function QuestionPageRaw() {
    ...
    const questionMain = useSelector((state) => {
        return state.questionMain.value;
    });
        ...
        for (let questionIndex = 0; questionIndex < questionContainers.length; questionIndex++) {
            for (let radioIndex = 0; radioIndex < questionContainers[questionIndex].childNodes.length; radioIndex++) {
                if (questionContainers[questionIndex].childNodes[radioIndex].checked) {
                    increaseWeight(questionRaw[questionIndex]["qWeight"][radioIndex]["type"], questionRaw[questionIndex]["qWeight"][radioIndex]["weight"]);
                }
            }
        }
    ...
    return (
        <>
            <div className="question-section-wrapper">
                {questionRaw.map((element, index) => {
                    return <Question type="raw" index={index} qString={questionRaw[index]["qString"]} qWeightArray={questionRaw[index]["qWeight"]}></Question>;
                })}
            </div>
